...
def main() raises:
    ...
    expect(
        r1.calls[0].arguments == '{"city": "Cluj"}',
        "1: args=" + r1.calls[0].arguments,
        all_ok,
    )
    expect(
        r1.content == "",
        "1: content should be empty, got [" + r1.content + "]",
        all_ok,
    )
    ...
    expect(
        r2.calls[0].name == "a" and r2.calls[1].name == "b", "2: names", all_ok
    )
    expect(
        r2.calls[1].arguments == "{}",
        "2: empty args=" + r2.calls[1].arguments,
        all_ok,
    )
    ...
    expect(
        r3.content == "Let me check.", "3: content=[" + r3.content + "]", all_ok
    )
    ...
    expect(
        r4.content == "The capital of Romania is Bucharest.",
        "4: content",
        all_ok,
    )
    ...
    expect(
        r5.content.find("not json") >= 0,
        "5: malformed kept in content=[" + r5.content + "]",
        all_ok,
    )
    ...
    var r6 = parse_tool_calls(
        String(
            '<tool_call>\n{"name": "question", "arguments": {"questions": ["How'
            ' many fingers does an octopus have?"}}\n</tool_call>'
        )
    )
    expect(r6.has_calls(), "6: repair should recover a call", all_ok)
    expect(
        len(r6.calls) == 1 and r6.calls[0].name == "question", "6: name", all_ok
    )
    expect(
        r6.calls[0].arguments.find("octopus") >= 0,
        "6: args kept=" + r6.calls[0].arguments,
        all_ok,
    )

    # 7. truncated: no closing </tool_call>, brackets left open -> repaired
    var r7 = parse_tool_calls(
        String(
            '<tool_call>\n{"name": "get_weather", "arguments": {"city": "Cluj"'
        )
    )
    expect(
        r7.has_calls() and r7.calls[0].name == "get_weather",
        "7: truncated recovered",
        all_ok,
    )
    expect(
        r7.calls[0].arguments.find("Cluj") >= 0,
        "7: args=" + r7.calls[0].arguments,
        all_ok,
    )
    ...
    expect(
        r8.has_calls() and r8.calls[0].name == "f",
        "8: trailing comma recovered",
        all_ok,
    )
    ...
    var g1 = parse_gemma_tool_calls(
        String(
            '<|tool_call>call:get_weather{city:<|"|>Paris<|"|>,days:3,detailed:true,units:<|"|>c<|"|>}<tool_call|>'
        )
    )
    expect(g1.has_calls() and len(g1.calls) == 1, "g1: one call", all_ok)
    expect(
        g1.calls[0].name == "get_weather",
        "g1: name=" + g1.calls[0].name,
        all_ok,
    )
    expect(
        g1.calls[0].arguments
        == '{"city": "Paris", "days": 3, "detailed": true, "units": "c"}',
        "g1: args=" + g1.calls[0].arguments,
        all_ok,
    )
    ...
    var g2 = parse_gemma_tool_calls(
        String(
            "<|tool_call>call:a{x:1}<tool_call|><|tool_call>call:noargs{}<tool_call|>"
        )
    )
    expect(len(g2.calls) == 2, "g2: two calls", all_ok)
    expect(
        g2.calls[0].name == "a" and g2.calls[1].name == "noargs",
        "g2: names",
        all_ok,
    )
    expect(
        g2.calls[0].arguments == '{"x": 1}',
        "g2: args0=" + g2.calls[0].arguments,
        all_ok,
    )
    expect(
        g2.calls[1].arguments == "{}",
        "g2: empty=" + g2.calls[1].arguments,
        all_ok,
    )
    ...
    var g3 = parse_gemma_tool_calls(
        String(
            '<|tool_call>call:nested{filter:{min:1,tags:[<|"|>a<|"|>,<|"|>b<|"|>]},q:<|"|>hi<|"|>}<tool_call|>'
        )
    )
    expect(
        len(g3.calls) == 1 and g3.calls[0].name == "nested", "g3: name", all_ok
    )
    expect(
        g3.calls[0].arguments
        == '{"filter": {"min": 1, "tags": ["a", "b"]}, "q": "hi"}',
        "g3: args=" + g3.calls[0].arguments,
        all_ok,
    )
    ...
    expect(
        len(g4.calls) == 1 and g4.content == "let me check",
        "g4: content=[" + g4.content + "]",
        all_ok,
    )
    ...
    expect(
        g5.content == "Paris is the capital of France.", "g5: content", all_ok
    )

    # G6. truncated: no closing <tool_call|> -> parse what's there
    var g6 = parse_gemma_tool_calls(
        String('<|tool_call>call:get_weather{city:<|"|>Cluj<|"|>')
    )
    expect(
        g6.has_calls() and g6.calls[0].name == "get_weather",
        "g6: truncated recovered",
        all_ok,
    )
    expect(
        g6.calls[0].arguments.find("Cluj") >= 0,
        "g6: args=" + g6.calls[0].arguments,
        all_ok,
    )
    ...
    var g7 = parse_gemma_tool_calls(
        String(
            "<|channel>thought\n<channel|>The weather in Paris is 22\xc2\xb0C"
            " and sunny."
        )
    )
    expect(not g7.has_calls(), "g7: no calls", all_ok)
    expect(
        g7.content == "The weather in Paris is 22\xc2\xb0C and sunny.",
        "g7: cleaned=[" + g7.content + "]",
        all_ok,
    )
    ...
    expect(
        g8.content == "The answer is 4.",
        "g8: content=[" + g8.content + "]",
        all_ok,
    )
    expect(
        g8.reasoning == "let me work it out",
        "g8: reasoning=[" + g8.reasoning + "]",
        all_ok,
    )
    expect(
        g7.reasoning == "", "g7: empty reasoning=[" + g7.reasoning + "]", all_ok
    )
